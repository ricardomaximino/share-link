...
@Component
public class AudioSignalHandler extends TextWebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		if ("receiver".equals(role)) {
			...
			if (room == null || room.getSender() == null || room.getReceiver() != null) {
				send(session, "expired", "This audio link is not active.");
				session.close();
				return;
			}
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage textMessage) throws Exception {
		...
		if (roomId == null) {
			send(session, "expired", "This audio link has expired.");
			return;
		}
		AudioRoom room = rooms.get(roomId);
		if (room == null) {
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		if (room != null && room.getSender() == session) {
			expire(roomId, "Sender left. Audio link expired.");
		}
	}

	private void expire(String roomId, String reason) throws IOException {
		AudioRoom room = rooms.remove(roomId);
		if (room == null) {
			return;
		}
