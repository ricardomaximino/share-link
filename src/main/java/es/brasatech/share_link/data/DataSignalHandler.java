...
@Component
public class DataSignalHandler extends TextWebSocketHandler {
	private final Map<String, DataRoom> rooms = new ConcurrentHashMap<>();
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		if ("receiver".equals(role)) {
			DataRoom room = rooms.get(roomId);
			if (room == null || room.getSender() == null || room.isUsed() || room.getReceiver() != null) {
				send(session, "expired", "This link is not active.");
				session.close();
				return;
			}
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		if (room != null && room.getSender() == session) {
			expire(roomId, "Sender left. Link expired.");
		}
	}

	private void expire(String roomId, String reason) throws IOException {
		DataRoom room = rooms.remove(roomId);
		if (room == null) {
			return;
		}
